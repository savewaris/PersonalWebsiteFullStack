...
/**
 * Agent State Manager — Multi-CLI, Cross-Computer Task Synchronizer & Real-Time Action Ledger
...
function loadState() {
  if (!existsSync(STATE_FILE)) {
    console.error('❌ State file not found at .agents/state/progress.json');
    process.exit(1);
  }
  try {
...
function ensureLogFiles() {
  if (!existsSync(SESSION_LOG_FILE)) {
    writeFileSync(SESSION_LOG_FILE, `# PersonalWebsite — Agent Session Handover Ledger\n\n`, 'utf8');
  }
  if (!existsSync(LIVE_STEP_LOG_FILE)) {
    writeFileSync(LIVE_STEP_LOG_FILE, `# PersonalWebsite — Real-Time Agent Action Ledger\n\n> Live chronological log of micro-actions performed by active agents.\n\n`, 'utf8');
  }
  if (!existsSync(LOCKS_FILE)) {
...
if (args.includes('--unlock')) {
  ...
  if (rawFiles.length === 0) {
    // Unlock all for current active task
...
if (args.includes('--step')) {
  ...
  if (!desc) {
    console.error('❌ Please specify step description (e.g. --step "Updated prisma schema")');
    process.exit(1);
  }
...
if (args.includes('--step-done')) {
  ...
  if (!desc) {
    console.error('❌ Please specify step description (e.g. --step-done "DB push succeeded")');
    process.exit(1);
  }
...
if (args.includes('--checkpoint')) {
  ...
  if (!summary) {
    console.error('❌ Please specify checkpoint summary (e.g. --checkpoint "Completed Issue #1 schema and UI")');
    process.exit(1);
  }
...
if (args.includes('--start')) {
  ...
  console.log(`   Assigned Agents: ${(task.assignedAgents || []).join(', ') || 'general-agent'}\n`);
  process.exit(0);
}
...
if (args.includes('--done')) {
  ...
  // Automatically release locks owned by this task
...
if (args.includes('--add')) {
  ...
  if (!title) {
    console.error('❌ Please specify task title (e.g. --add "New Feature")');
    process.exit(1);
  }
...
console.log('\n======================================================');
console.log('📊 PERSONALWEBSITE — ACTIVE AGENT & TASK STATE');
console.log('======================================================');
...
console.log('\n------------------------------------------------------');
console.log('⚡ RECENT LIVE MICRO-STEPS (.agents/state/LIVE_STEP_LOG.md):');
if (existsSync(LIVE_STEP_LOG_FILE)) {
  const lines = readFileSync(LIVE_STEP_LOG_FILE, 'utf8')
    .split('\n')
  ...
  if (lines.length > 0) {
    ...
    console.log('  (No live actions logged yet in this cycle)');
  }
}

console.log('\n------------------------------------------------------');
console.log(`🩺 Health Status:     ${state.lastDoctorCheck?.passed ? 'ALL CHECKS PASSED (' + state.lastDoctorCheck.totalChecks + '/' + state.lastDoctorCheck.totalChecks + ')' : 'UNKNOWN'}`);
console.log('======================================================\n');
