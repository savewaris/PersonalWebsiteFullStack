...
Use this skill whenever creating, updating, or reviewing UI components, pages, or styling across the application to ensure complete adherence to global UX/UI standards without manual checking.
...
3. **Responsive Viewport Integrity**:
   ...
   - Fluid typography and flexible grid cards (`minmax(auto-fit, ...)`).
4. **Touch & Interaction Ergonomics**:
   ...
   - Adequate tap spacing to avoid misclicks on mobile.
5. **Motion Safety**:
...
| **Missing Image Alt** | `<img>` tag without description | Add descriptive `alt="Project preview thumbnail"` or `alt="" aria-hidden="true"` if purely decorative. |
