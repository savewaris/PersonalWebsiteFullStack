...
This skill provides step-by-step procedures for safely refactoring monolithic components and data layers into atomic, isolated domain modules without regressions.

---

## 1. Decomposing Monolithic Data Access Layers

When refactoring a shared data layer (e.g. `src/lib/data.ts`):
1. **Acquire File Locks**:
   ```bash
   npm run agent:state -- --lock src/lib/data.ts src/lib/data/ --reason "Modularizing data fetchers"
   ```
2. **Create Domain Fetcher Files** inside `src/lib/data/`:
   - `skills.ts` -> `getSkills()`
...
When decoupling composite sections (e.g. `LanguagesInterestsSection.tsx`):
...
3. Update parent container (`src/app/page.tsx`) to compose the new atomic sections cleanly.
...
If another active session is modifying a locked file:
